...
class EEG_Data:
    def __init__(self, config_file_path, subject_directory, xdf_file_path):
        ...
        eeg_pos = np.where(streams_info == config['general-settings']['lsl-streams']['eeg']['name'])[0][0]
        marker_pos = np.where(streams_info == config['general-settings']['lsl-streams']['marker']['name'])[0][0]
        eeg_stream = streams[eeg_pos]
        marker_stream = streams[marker_pos]
        eeg_with_label = add_class_labels(eeg_stream, marker_stream)
        #eeg_signal = eeg_with_label['time_series']

        #self.eeg_signal = eeg_signal * 1e-6
        self. eeg_instants = eeg_stream['time_stamps']
        self.data_eeg = eeg_with_label.T
