...
class Measurement_Data:
    ...
    def create_epochs(self, signal, visualize_epochs=True, rois=True, set_annotations=True):
        ...
        #self.epochs.save(self.dir_epochs + '/run' + str(self.n_run) + '-epo.fif', overwrite=True)
    ...
    def plot_erds_maps(self, picks=['C3', 'Cz', 'C4'], show_epochs=False, show_erds=True, cluster_mode=False,
                       preproc_data=False, load_epochs=False, tfr_mode=False):

        if load_epochs == True:
            epochs = mne.read_epochs('C:/2D3D_Analysis/Results/S14_sesvr-epo.fif')

        elif preproc_data == True:
    ...
    def run_preprocessing_to_epoch(self):
        ...
        #self.visualize_raw()
        self.raw_filt.save(self.dir_epochs + '/run' + str(self.n_run) + '-preproc-raw.fif', overwrite=True)
        ...
        self.create_epochs(visualize_epochs=True)
        self.epochs.save(self.dir_epochs + '/run' + str(self.n_run) + '-epo.fif', overwrite=True)

        #self.create_evoked()
        #self.visualize_evoked()

        mne.viz.plot_topomap(self.epochs, self.info)
    ...
    def run_preprocessing_to_evoked(self, ses_key, freq_band=False):
        self.create_raw()
        self.raw_filt = self.raw.copy()
        ...
        self.raw_filt.save(self.dir_subj_interim + 'ses' + ses_key + 'run' + str(self.n_run) +
                           '-preproc-raw.fif', overwrite=True)

        if freq_band:
            signal_alpha = self.get_frequency_band([8, 12])
            self.create_epochs(signal_alpha, visualize_epochs=False)
            self.epochs.save(self.dir_subj_interim + 'ses' + ses_key + 'run' + str(self.n_run) + '_alpha-epo.fif',
                             overwrite=True)

            signal_beta = self.get_frequency_band([16, 24])
            self.create_epochs(signal_beta, visualize_epochs=False)
            self.epochs.save(self.dir_subj_interim + 'ses' + ses_key + 'run' + str(self.n_run) + '_beta-epo.fif',
                             overwrite=True)
        else:
            signal = self.raw_filt
            self.create_epochs(signal, visualize_epochs=False)
            self.epochs.save(self.dir_subj_interim + 'ses' + ses_key + 'run' + str(self.n_run) + '-epo.fif',
                         overwrite=True)

        # only latest epochs (beta) are used for evoked and returned!
        self.evoked_left = self.epochs['1'].average()
        #self.evoked_left.plot_topomap(ch_type="eeg")
        self.evoked_right = self.epochs['2'].average()
        #self.evoked_right.plot_topomap(ch_type="eeg")

        #self.create_evoked(rois=False)
        #self.visualize_evoked()

        #mne.viz.plot_topomap(self.epochs, self.info)

        return self.evoked_left, self.evoked_right
