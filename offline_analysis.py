...
class EEG_Signal:
    ...
    def extract_epochs(self):
        data_labels = self.data_eeg[0, :]
        data = self.data_eeg[1:self.n_ch+1, :]
        indexes = np.where((data_labels == 121) | (data_labels == 122))[0]

        for idx in indexes:
            self.epochs.append(data[:, idx:idx + self.n_samples_task])
    ...
    def analyze_eeg(self):
        ...
        eeg_scaled = self.data_eeg * 20e-3
        eeg_scaled[0:] = self.data_eeg[0:]

        # plot raw data per trial

        #'''
        for i in range(3, 4): # for i in range(len(indexes_class_all)):
            raw = mne.io.RawArray(
                eeg_scaled[1:self.n_ch + 1, self.indexes_class_all[i] - self.n_ref:self.indexes_class_all[i] + self.n_samples_trial], self.info)
        ...
        # picks = ['F3', 'F4']
        picks = ['C3', 'C4']
        picked_channels = [self.raw.ch_names.index(ch) for ch in picks]
        #picks = ['P3', 'P4']

        eeg_epoched = EEG_Signal.extract_epochs(self)
        n_ep, n_ch, n_s = np.shape(eeg_epoched)
        events_array = np.column_stack((np.arange(0, n_ep*n_s, n_s), np.zeros(n_ep, dtype=int), np.array(self.class_labels, dtype=int)))
        #eeg_epochs_array = mne.EpochsArray(eeg_epoched, self.info, self.duration_ref, events=events_array, event_id=self.event_dict) # funktioniert nicht #Todo
        #eeg_epochs_array.plot(picks=picks, show_scrollbars=True, block=True, events=events_array, event_id=self.event_dict, n_epochs=5) # Signal ist zu klein um sichtbar zu sein #Todo
        ...
        epochs = mne.Epochs(self.raw, self.events, self.event_dict, tmin - 0.5, tmax + 0.5, picks=picks, baseline=None, preload=True)
        epochs.plot(picks=picks, show_scrollbars=True, events=self.events, event_id=self.event_dict)
        ...
        for event in self.event_dict:
            ...
            fig.suptitle(f"ERDS - {event} hand {self.motor_mode} run {self.n_run}")
